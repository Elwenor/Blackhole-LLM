...
def predict_and_decode_answer(model, question, vocab, determined_numeric_feature_dim, max_seq_len, device,
                              max_output_tokens=50):
    """
    Generates an answer from the model for a given question and decodes it.
    NOTE: This is an auto-regressive (token-by-token) generation process,
    which is significantly slower than batched training/evaluation.
    For large datasets, this should only be used for a small sample.
    """
    model.eval()
    ...
    enc_tokens, enc_number_map = tokenize(question)
    enc_token_ids = [vocab.get(tok, vocab.get('<|unk|>', 0)) for tok in enc_tokens]
    
    _enc_numeric_features = []
    padded_feat_row_list = [-2.0] * determined_numeric_feature_dim
    for token_idx in range(len(enc_token_ids)):
        if token_idx in enc_number_map:
            val, typ, raw = enc_number_map[token_idx]
            _enc_numeric_features.append(number_embedding_features(val, typ, dim=determined_numeric_feature_dim).tolist())
        else:
            _enc_numeric_features.append(padded_feat_row_list)
    ...
    if enc_len > max_seq_len:
        ...
        num_padding_needed_enc = max_seq_len - enc_len
        enc_token_ids += [vocab.get('<|pad|>', 0)] * num_padding_needed_enc
        _enc_numeric_features += [padded_feat_row_list] * num_padding_needed_enc
        enc_attention_mask += [True] * num_padding_needed_enc

    # Convert to tensors (add batch dimension of 1)
    encoder_token_ids = torch.tensor([enc_token_ids], dtype=torch.long, device=device)
    encoder_numeric_features = torch.tensor([_enc_numeric_features], dtype=torch.float32, device=device)
    encoder_attention_mask = torch.tensor([enc_attention_mask], dtype=torch.bool, device=device)

    # Initialize decoder input with <|bos|>
    decoder_input_token_ids = torch.tensor([[vocab.get('<|bos|>', 0)]], dtype=torch.long, device=device)
    decoder_input_numeric_features = torch.tensor([[padded_feat_row_list]], dtype=torch.float32, device=device)
    decoder_attention_mask = torch.tensor([[False]], dtype=torch.bool, device=device) # Initial mask for BOS

    predicted_token_ids = []
    predicted_num_features_list = [] # Store as list of numpy arrays or lists

    with torch.no_grad():
        # Encode the question once
        enc_token_emb = model.token_embedding(encoder_token_ids)
        enc_num_emb = model.num_embedding(encoder_numeric_features)
        encoder_input_emb = model.token_to_common_dim(enc_token_emb) + model.num_to_common_dim(enc_num_emb)
        encoder_output = model.transformer_encoder(encoder_input_emb, src_key_padding_mask=encoder_attention_mask)

        for _ in range(max_output_tokens):
            # Generate target mask for auto-regressive decoding
            tgt_seq_len = decoder_input_token_ids.size(1)
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(tgt_seq_len).to(device)

            # Pass through decoder
            dec_token_emb = model.token_embedding(decoder_input_token_ids)
            dec_num_emb = model.num_embedding(decoder_input_numeric_features)
            decoder_input_emb = model.token_to_common_dim(dec_token_emb) + model.num_to_common_dim(dec_num_emb)
            ...
            token_logits = model.token_head(decoder_output[:, -1:]) # (batch_size, 1, vocab_size)
            num_feature_output = model.num_head(decoder_output[:, -1:]) # (batch_size, 1, feature_dim)

            # Select the most probable token
            next_token_id = token_logits.argmax(dim=-1).squeeze(0).item()
            
            # Ensure next_num_feature is a (1, 1, feature_dim) tensor for concatenation
            next_num_feature_tensor = num_feature_output.squeeze(0).unsqueeze(0) # (1, feature_dim) -> (1, 1, feature_dim)
            predicted_num_features_list.append(next_num_feature_tensor.squeeze(0).cpu().numpy().tolist()) # Store for decoding

            predicted_token_ids.append(next_token_id)

            # Stop if EOS token is predicted
            if next_token_id == vocab.get('<|eos|>', -1):
                break

            # Append the predicted token and numeric features to decoder input for the next step
            decoder_input_token_ids = torch.cat(
                [decoder_input_token_ids, torch.tensor([[next_token_id]], dtype=torch.long, device=device)], dim=1
            )
            # If the next token is a number, we use its predicted features for the next step.
            # Otherwise, we use the padded_feat_row.
            if next_token_id == vocab.get('<|num|>'):
                decoder_input_numeric_features = torch.cat(
                    [decoder_input_numeric_features, next_num_feature_tensor], dim=1
                )
            else:
                decoder_input_numeric_features = torch.cat(
                    [decoder_input_numeric_features, padded_feat_row.unsqueeze(0).unsqueeze(0)], dim=1
                )
            
            decoder_attention_mask = torch.cat(
                [decoder_attention_mask, torch.zeros((1, 1), dtype=torch.bool, device=device)], dim=1
            )
            # Truncate if it exceeds max_seq_len
            if decoder_input_token_ids.size(1) > max_seq_len:
                decoder_input_token_ids = decoder_input_token_ids[:, -max_seq_len:]
                decoder_input_numeric_features = decoder_input_numeric_features[:, -max_seq_len:]
                decoder_attention_mask = decoder_attention_mask[:, -max_seq_len:]


    # Decode the predicted sequence
    decoded_answer_tokens = []
    current_num_feature_idx = 0
    for i, token_id in enumerate(predicted_token_ids):
        token = idx_to_token.get(token_id, '<|unk|>')
        if token == '<|num|>':
            if current_num_feature_idx < len(predicted_num_features_list):
                try:
                    # decode_number_from_features expects numpy array
                    decoded_num_val, _ = decode_number_from_features(np.array(predicted_num_features_list[current_num_feature_idx]), determined_numeric_feature_dim)
                    # Format for cleaner numbers, e.g., 5.00 -> 5, 5.20 -> 5.2
                    # Check if the number is essentially an integer
                    if abs(decoded_num_val - round(decoded_num_val)) < 1e-6:
                        decoded_answer_tokens.append(str(int(round(decoded_num_val))))
                    else:
                        decoded_answer_tokens.append(f"{decoded_num_val:.2f}".rstrip('0').rstrip('.'))
                except Exception as e:
                    decoded_answer_tokens.append('<|num_err|>') # Indicate a number decoding error
                    # print(f"Warning: Error decoding number: {e}") # Debugging
            else:
                decoded_answer_tokens.append('<|num_missing|>') # Fallback if features are missing
            current_num_feature_idx += 1
        elif token == '<|cap|>':
            # Handle capitalization for the next actual word token
            if i + 1 < len(predicted_token_ids):
                next_raw_token = idx_to_token.get(predicted_token_ids[i+1], '<|unk|>')
                if next_raw_token not in ['<|cap|>', '<|allcaps|>', '<|num|>', '<|bos|>', '<|eos|>', '<|pad|>', '<|unk|>', '<|space|>']:
                    decoded_answer_tokens.append(next_raw_token.capitalize())
                    i += 1 # Skip the next token as it's been processed
                # else: just drop <|cap|> if no word follows or it's another special token
        elif token == '<|allcaps|>':
            # Handle all caps for the next actual word token
            if i + 1 < len(predicted_token_ids):
                next_raw_token = idx_to_token.get(predicted_token_ids[i+1], '<|unk|>')
                if next_raw_token not in ['<|cap|>', '<|allcaps|>', '<|num|>', '<|bos|>', '<|eos|>', '<|pad|>', '<|unk|>', '<|space|>']:
                    decoded_answer_tokens.append(next_raw_token.upper())
                    i += 1 # Skip the next token as it's been processed
                # else: just drop <|allcaps|> if no word follows or it's another special token
        elif token == '<|space|>':
            decoded_answer_tokens.append(' ')
        elif token in ['<|bos|>', '<|eos|>', '<|pad|>']:
            continue # Skip special tokens in the final output
        else:
            decoded_answer_tokens.append(token)
    
    # Reconstruct the string
    predicted_answer = "".join(decoded_answer_tokens).replace('<|space|>', ' ').strip()
    predicted_answer = re.sub(r'\s([.,!?;:])', r'\1', predicted_answer) # Fix space before punctuation
    predicted_answer = re.sub(r'\s+', ' ', predicted_answer).strip() # Normalize multiple spaces
