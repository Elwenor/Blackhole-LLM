...
# Te importy będą teraz działać poprawnie po naprawieniu cyklicznych zależności w modeling_nova.py
from blackhole.embadding_hugging_face.hugging_embedding import BlackholeEmbeddings
...
config = BlackholeConfig(
    ...
    # numeric_input_features jest obliczane automatycznie przez BlackholeConfig
    numeric_projection_intermediate_size=256,
...
def verify_numeric_embedding(
    ...
    if num_token_pos.numel() > 0:
        ...
        # Użyj np.isnan dla wartości numerycznych, ponieważ mogą zawierać NaN dla paddingu
        if (np.isnan(actual_val) and not np.isnan(expected_num_value)) or \
           (not np.isnan(actual_val) and not np.isclose(actual_val, expected_num_value)) or \
           actual_format != expected_format_id:
