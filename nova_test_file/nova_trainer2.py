...
# Corrected imports to use relative paths within the 'nova_test_file' package
from .modeling_nova import BlackholeForMaskedLM, BlackholeForSequenceClassification
from .configuration_nova import BlackholeConfig
from .data_collator2 import BlackholeDataCollatorForSeq2Seq as BlackholeDataCollatorForLanguageModeling

# Corrected path for the tokenizer import
from .hugging_tokenizer2 import BlackholeTokenizer2
...
class TrainingArguments:
    ...
    per_device_train_batch_size: int = field(
        default=8, metadata={"help": "Rozmiar batcha dla treningu na każdym urząd..."}
    )
    output_dir: str = field(
        metadata={"help": "Katalog wyjściowy dla checkpointów modelu i wyników treningu."}
    )
...
class NovaTrainer:
    ...
    def __init__(
        ...
        tokenizer: Optional[BlackholeTokenizer2] = None, # ZMIANA: Użyj BlackholeTokenizer2
    ...
    def load_model(self, model_path: str):
        ...
        if self.tokenizer: # Użyj istniejącego tokenizatora, jeśli dostępny
            self.tokenizer = BlackholeTokenizer2.from_pretrained(model_path) # ZMIANA: Użyj BlackholeTokenizer2
